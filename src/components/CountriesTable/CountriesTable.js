...
const SortArrow = ({ direction }) => {
  if (direction === 'asc') {
    return (<div className={styles.heading_arrow}><KeyboardArrowUpRounded color="inherit" /></div>);
  }
  if (direction === 'desc') {
    return (<div className={styles.heading_arrow}><KeyboardArrowDownRounded color="inherit" /></div>);
  } else {
    return <></>;
  }
};
...
const CountriesTable = ({ countries }) => {
  ...
  const setValueAndDirection = (_value) => {
    if (!direction) {
      setDirection('desc');
    } else if (direction === 'desc') {
      setDirection('asc');
